...
(LOG_SEQ.NEXTVAL, NULL, 'SAVINGS', NULL, CURRENT_TIMESTAMP);


IF (SELECT ACCOUNT_NUMBER, AMOUNT FROM BANK_ACCOUNTS 
WHERE ACCOUNT_NUMBER = P_ACCOUNT_NUMBER)
THEN
UPDATE BANK_ACCOUNTS
SET BALANCE = BALANCE + P_AMOUNT
...
CREATE OR REPLACE PROCEDURE MAKE_WITHDRAWAL (p_account_number IN VARCHAR2, p_amount IN NUMBER, P_LOG OUT VARCHAR2)
...
IF (SELECT ACCOUNT_NUMBER, AMOUNT FROM BANK_ACCOUNTS THEN
UPDATE BANK_ACCOUNTS
SET BALANCE = BALANCE + P_AMOUNT
WHERE ACCOUNT_NUMBER = P_ACCOUNT_NUMBER;
AND BALANCE > 1000;
